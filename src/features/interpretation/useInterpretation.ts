...
import type { InterpretationResult, InterpretationRequest } from '@services/ai/interpretation/InterpretationService';
import { InterpretationLimitError, ConsentRequiredError } from '@services/ai/interpretation/InterpretationService';
...
export function useInterpretation() {
  ...
  const interpret = useCallback(async (request: InterpretationRequest) => {
  ...
  const retry = useCallback((request: InterpretationRequest) => {
    interpret(request);
  }, [interpret]);
