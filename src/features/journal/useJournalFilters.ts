 ...
 * its loading state then, so the filter sheet isn't reachable yet. */
export function useJournalFilters(userId: string | null) {
  ...
  const applyFilters = useCallback(
    (newFilters: JournalFilters) => {
      ...
      try {
        ...
        const conditions: string[] = [JOURNAL_ENTRY_SCOPE];
