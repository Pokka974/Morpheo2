...
export const dreams = sqliteTable('dreams', {
  ...
  lastModifiedAt: text('last_modified_at').notNull().default(sql`(datetime('now'))`),
  isDeleted: integer('is_deleted', { mode: 'boolean' }).notNull().default(false),
  editedSinceInterpretation: integer('edited_since_interpretation', { mode: 'boolean' }).notNull().default(false),
  syncStatus: text('sync_status', { enum: ['local', 'sync_pending', 'synced', 'sync_failed'] }).notNull().default('local'),
});
...
export const media = sqliteTable('media', {
  ...
  generationStatus: text('generation_status', { enum: ['pending', 'processing', 'complete', 'failed', 'safety_blocked'] }).notNull().default('pending'),
...
export const recurrencePatterns = sqliteTable('recurrence_patterns', {
  ...
  patternType: text('pattern_type', { enum: ['keyword', 'emotion', 'cultural_reference'] }).notNull(),
