...
- [X] T077 [US5] Implement `DallEImageGenerationService` in `src/services/ai/image/DallEImageGenerationService.ts`: calls `POST /functions/v1/generate-image`; maps 200 → `MediaResult`; maps 400 `safety_blocked` → `ContentSafetyError`; maps 409 `regen_limit_reached` → `RegenerationLimitError`; maps 429 → `ImageLimitError`; after success, calls `StorageService.cacheMedia()` to cache locally
...
- [X] T079 [US5] Write unit tests for `DallEImageGenerationService` in `tests/unit/services/ai/DallEImageGenerationService.test.ts`: mock Edge Function invocation; test successful generation returns `MediaResult` with `generation_status='complete'`; test content safety block throws `ContentSafetyError('input')`; test regen limit throws `RegenerationLimitError(3)` for free user
