...
serve(async (req: Request) => {
  ...
  const { data: interpretations } = await supabase
    ...
    .select('dream_id, overall_reading, keywords, emotions, cultural_references, created_at, prompt_version')
    .eq('user_id', user.id)
  ...
  await supabase.storage
    .from('dream-media')
    .upload(storagePath, new TextEncoder().encode(json), { contentType: 'application/json', upsert: false });
